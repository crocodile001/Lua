...
-- 對話文本 ------------------------------------------
      local txt ={
        "沈安：\n村長應該是村子裡最了解命案狀況的人，先去問問村長再去現場調查好了",
        "\n\n----------文末(場景互動)----------"
      }
     
      local txt2 ={
        "\n\n我是物品欄喔",
        "沈安：\n我是調查局派來的調查員沈安，負責調查前幾天發生的命案，你就是村長吧，麻煩你跟我說一下大致情況。",
        "村長：\n是在七月一號早上被村民發現死在河裡，應該是孩子貪玩想玩水，沒想到卻被銀狐大人索了命\n",
        "沈安：\n\n我們去河邊調查看看"
      }
...
-- 顯示文本 -------------------------------------------------------------- 
      ...
      local function press_next(event)
        if (lock==0 and lock2 == 0 and lock3 == 0 and i~=2) then
...
---- 按鈕事件 ---------------------------------------------
      ...
      local talk = function(event)
        ...
        timer.performWithDelay(5000,unlock)
      end
      ...
      local buttonHandler = function( event )
        if (lock == 0 and lock2 == 0 and lock3 == 0) then
          ...
          elseif (event.target.id == "village_headman" and i==2) then
            ...
            timer.performWithDelay(6000,talk)
          end
          
          if( event.target.id == "door" and village_head == 1 ) then
            j = 1;  ii = 4;   lock2 = 1;  lock3 = 1
...
--      button4.x = display.contentCenterX-130; button4.y = display.contentCenterY+30; button4.width = 35; button4.height = 30; sceneGroup:insert(button4)
      person1.x = display.contentCenterX+50; person1.y = display.contentCenterY- 20; person1.width = 150; person1.height = 225; sceneGroup:insert(person1)
      person2.x = display.contentCenterX+ 170; person2.y = display.contentCenterY- 10; person2.width = 100; person2.height = 150; sceneGroup:insert(person2)
-------------------------------------------------------------------------
