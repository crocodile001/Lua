...
-- 人物圖片 ------------------------------------------------------------------------------------
      ...
      main_actor.x = display.contentCenterX - 0
...
-- 對話文本 ------------------------------------------
      ...
      -- 文本顯示變數
      local i = 1         -- txt 的 conuter
      local ii = 1        -- txt2 的 counter
      local j = 1         -- txt 和 txt2 的共用 counter
      local lock = 1      -- lock next 之外的按鈕
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then  --場景在螢幕卸載時立刻執行
        ...
        composer.removeScene("level_1_1")
    end
end
