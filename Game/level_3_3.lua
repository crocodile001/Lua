...
-- 背景圖 ----------------------------------------------------------------------------------
    local background = display.newImageRect( sceneGroup, "images/農民家.jpg", 580, 340 )
...
-- 人物圖片 ------------------------------------------------------------------------------------    
      local main_actor = display.newImageRect(sceneGroup, 'images/沈安.png', 150, 225)
      main_actor.x = display.contentCenterX + 120
...
-- 對話文本 ------------------------------------------
      ...
      local txt2 ={
        ...
        "\n\n( 獲得線索X1 )",
        "\n\n( 獲得線索X1 )",
        "沈安：\n\n桌上有兩副碗筷，可是死者是獨居，莫非有其他人來過？",
        "沈安：\n\n杯子裡有粉末，看這顏色、味道，應該是安眠藥",
        "沈安：\n\n有一本日記",
        "沈安：\n(小諾的日記)\n\n沈安是很厲害的獵人，他不只懂得打獵、他還懂很多事喔! ",
        "沈安：\n\n4點44分",
        "沈安：\n事情越來越蹊蹺了，接連死的兩個人都符合童謠的內容，而這童謠失傳已久，那人卻會唱，太奇怪了，我覺得這件事一定跟那個人脫不了關係。",
        "沈安：\n我記得她總是戴著狐狸面具，也許可以從面具找線索。你們這裡有賣面具的店嗎？",
        "村長：\n\n直走遇到路口左轉就有一家面具店，你可以去問問看。",
        "我看他常常在一些工具上綁一個結，他說這是稱人結，是繩結之王，主要用在救人，但還可以用在很多地方。我總是在旁邊偷偷學，現在，這是我唯一會打得結。每想沈安一次，我就在一個地方打上這種結。我真的真的好想他……"
      }
...
---- 按鈕事件 ---------------------------------------------
      ...
      local talk6 = function(event)
        ...
        timer.performWithDelay(5000,talk7)
      end
      ...
      local talk3 = function(event)
        ...
        timer.performWithDelay(5500,talk8)
      end
      ...
      local buttonHandler = function( event )
        if (lock == 0 and lock2 == 0 and lock3 == 0) then
          ...
          elseif (event.target.id == "village_headman" and Dinner_table == 1 and Cup == 1 and Diary == 1) then
            ...
            lock3 = 1
            timer.performWithDelay(6000,talk6)
            
          elseif (event.target.id == "dinner_table") then
            j = 1;  ii = 2;   lock2 = 1
            timer.performWithDelay(50,iter2,utf8.len(txt2[2]))
            local clue1 = display.newImageRect("images/餐桌.png", 30, 25)
            clue1.x=display.contentCenterX-235
            clue1.y=display.contentCenterY+30
            sceneGroup:insert(clue1)
            Dinner_table = 1;  lock3 = 1
            timer.performWithDelay(1500,talk)
          elseif( event.target.id == "cup") then
            j = 1;  ii = 3;   lock2 = 1
            timer.performWithDelay(50,iter2,utf8.len(txt2[3]))
            local clue2 = display.newImageRect("images/安眠藥水杯.png", 30, 25)
          ...
          elseif( event.target.id == "drawer") then
            lock3 = 1;drawer=1
            timer.performWithDelay(0,talk4)
          elseif( event.target.id == "diary" and drawer == 1) then
            j = 1;  ii = 4;   lock2 = 1
            timer.performWithDelay(50,iter2,utf8.len(txt2[4]))
            local clue3 = display.newImageRect("images/日記本.png", 30, 25)
          ...
          elseif( event.target.id == "clock") then
            lock3 = 1
            timer.performWithDelay(0,talk5)
          end
...
--      button4.x = display.contentCenterX-130; button4.y = display.contentCenterY+30; button4.width = 35; button4.height = 30; sceneGroup:insert(button4)
      person1.x = display.contentCenterX+200; person1.y = display.contentCenterY- 20; person1.width = 150; person1.height = 225; sceneGroup:insert(person1)
      person2.x = display.contentCenterX+0; person2.y = display.contentCenterY; person2.width = 120; person2.height = 120; sceneGroup:insert(person2)
      person3.x = display.contentCenterX+10; person3.y = display.contentCenterY-40; person3.width = 40; person3.height = 50; sceneGroup:insert(person3)
      person4.x = display.contentCenterX-100; person4.y = display.contentCenterY-60; person4.width = 50; person4.height = 60; sceneGroup:insert(person4)
      person5.x = display.contentCenterX-100; person5.y = display.contentCenterY-0; person5.width = 90; person5.height = 90; sceneGroup:insert(person5)
      person6.x = display.contentCenterX-200; person6.y = display.contentCenterY-100; person6.width = 60; person6.height = 70; sceneGroup:insert(person6)
-------------------------------------------------------------------------
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then  --場景在螢幕卸載時立刻執行
        ...
        composer.removeScene("level_3_3")
    end
end
