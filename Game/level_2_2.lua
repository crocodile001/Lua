...
-- 人物圖片 ------------------------------------------------------------------------------------    
      ...
      main_actor.y = display.contentCenterY + 10
------------------------------------------------------------------------------------------------
...
-- 對話文本 ------------------------------------------
      ...
      local txt2 ={
        "\n\n我是物品欄喔",
        "沈安：\n我發現從五年前開始，七月初一到七月初九這段期間都會有三個人死亡，死亡方式感覺跟一首童謠有關。那是我進到銀狐村前聽到一個人唱的",
        "沈安：\n我記得是這樣：「七月初一鬼門開，銀狐大人過小橋。橋下河水流啊流，河裡飄個傻姑娘；七月初五花兒香，銀狐大人巡庄稼。田中有棵大榕樹，樹上吊個老農民」",
        "沈安：\n那時候她唱到一半被我打斷了，我擔心接下來還會有人死掉，您知道下一段的內容嗎？",
        "村長：\n這童謠我有聽說過，但歌詞失傳已久，大概村子裡是沒有人會唱。"
      }
...
---- 按鈕事件 ---------------------------------------------
      ...
      local talk3 = function(event)
        ...
        timer.performWithDelay(4000,goto)
      end

      local talk2 = function(event)
        --village_headmanr:setFillColor(0.5,0.5,0.5)
        audio.play( btn, {channel=2,1} )
        j = 1;  ii = 4;   lock2 = 1
        timer.performWithDelay(50,iter2,utf8.len(txt2[4]))
        timer.performWithDelay(5000,talk3)
      end

      local talk = function(event)
        --village_headman:setFillColor(0.5,0.5,0.5)
        audio.play( btn, {channel=2,1} )
        j = 1;  ii = 3;   lock2 = 1
        timer.performWithDelay(50,iter2,utf8.len(txt2[3]))
        timer.performWithDelay(7000,talk2)
      end
...
--      button4.x = display.contentCenterX-130; button4.y = display.contentCenterY+30; button4.width = 35; button4.height = 30; sceneGroup:insert(button4)
      person1.x = display.contentCenterX+80; person1.y = display.contentCenterY- 20; person1.width = 175; person1.height = 250; sceneGroup:insert(person1)
-------------------------------------------------------------------------
